...
class Loom(object):
  ...
  def _setup_loom_ops(self, named_ops):
    ...
    if self._use_tensor_array:
      self._loom_op_names = sorted(six.iterkeys(named_ops))
      self._loom_ops = [named_ops[k] for k in self._loom_op_names]
    else:
      ...
      #
      # The first len(self._type_shapes) ops are forced to be the passthrough
      # ops for the corresponding TypeShape.  This is enforced in
      # VerifyLoomMetadata.
      self._loom_op_names = (
    ...
    # _loom_total_outputs: The sum of the numebr of outputs across all loom ops.
    self._loom_total_outputs = sum(
        len(op.output_type_shapes) for op in self._loom_ops)
  ...
  def _setup_network(self):
    ...
    if self._direct_feed_dict:
      ...
      self._num_results_per_type_shape = tf.placeholder(
          TENSOR_IDX_T, name='num_results_per_type_shape')
    else:
      ...
      (self._arg_wiring_concat,
       ...
       self._output_wirings,
       self._constants,
       self._num_results_per_type_shape) = self._weaver_op(
    ...
    # TypeShape.
    #
    # _num_results_per_type_shape: Number of LoomResults that are output by
    # calls to LoomOps, for each TypeShape. Only used if self._use_tensor_array
    # is true.
    ...
    state = []
    if self._use_tensor_array:
      # TODO(rshin): Create separate TensorArrays for LoomResults which will be
      # used only once vs LoomResults which get used many times, to take
      # advantage of clear_after_read.
      # If reference counting is added to TensorArray, then we can also do
      # something different.
      for i, (inputs_tensor, named_tensors) in (
          enumerate(zip(inputs, self._ts_idx_to_named_tensors))):
        inputs_tensor_size = tf.shape(inputs_tensor)[0]

        ta = tf.TensorArray(
            self._type_shapes[i].dtype,
            size=tf.add_n([self._num_results_per_type_shape[i],
                           inputs_tensor_size] +
                          ([len(named_tensors)] if named_tensors else [])),
            clear_after_read=False,
            element_shape=self._type_shapes[i].shape)

        # Here we need to know how many LoomOp-output LoomResults exist,
        # for each of the tensors.
        offset = self._num_results_per_type_shape[i]
        if named_tensors:
          named_tensors_size = tf.shape(named_tensors)[0]
          inputs_tensor_start = offset + len(named_tensors)

          ta = ta.scatter(
              tf.range(offset, inputs_tensor_start),
              tf.stack(named_tensors))
          ta = ta.scatter(
              tf.range(inputs_tensor_start,
                       inputs_tensor_start + inputs_tensor_size),
              inputs_tensor)
        else:
          ta = ta.scatter(
              tf.range(offset, offset + inputs_tensor_size),
              inputs_tensor)

        state.append(ta)

      state += [tf.zeros((), dtype=TENSOR_IDX_T) for _ in self._type_shapes]
    else:
    ...
    if self._max_depth == -1:  # For dynamic max_depth we use tf.while.
      ...
      def loop_body(depth, *state):
        ...
        if self._use_tensor_array:
          new_state = self._construct_loom_layer_tensor_array(depth, state)
        else:
      ...
      for depth in xrange(1, self._max_depth+1):
        with tf.name_scope('loom_depth_%03d' % depth):
          if self._use_tensor_array:
            state = self._construct_loom_layer_tensor_array(depth, state)
          else:
    ...
    with tf.name_scope('output_gathers'):
      if self._use_tensor_array:
        self._output = [
            ta.gather(w, name=self._type_shapes[ts_idx].tensor_flow_name())
            for ts_idx, (ta, w) in enumerate(
              zip(state[:len(self._type_shapes)], self._output_wirings))]
      else:
        self._output = [
  ...
  def _compute_arg_wiring(self, start_wire_pos, wire_pos_offset):
  ...
  def _sanity_check_op_output(self, op_idx, output_idx, output, output_ts):
      ...
      with tf.name_scope(self._loom_op_names[op_idx]):
        ...
        for arg_idx, arg_ts in enumerate(op.input_type_shapes):
          with tf.name_scope('arg_%d' % arg_idx):
            arg_wiring = self._compute_arg_wiring(
                start_wire_pos, wire_pos_offset)
        ...
        for output_idx, (output, output_ts) in enumerate(
          ...
          self._sanity_check_op_output(op_idx, output_idx, output, output_ts)
  ...
  def _construct_loom_layer_tensor_array(self, depth, state):
    # Note: `start_wire_pos` might be a tensor or an integer.
    start_wire_pos = (depth - 1) * self._loom_total_args

    wire_pos_offset = 0  # `wire_pos_offset` is an integer.

    tensor_arrays = list(state[:len(self._type_shapes)])
    output_offsets = list(state[len(self._type_shapes):])
      ...
      with tf.name_scope(self._loom_op_names[op_idx]):
        ...
        for arg_idx, arg_ts in enumerate(op.input_type_shapes):
          with tf.name_scope('arg_%d' % arg_idx):
            ...
            #arg_wiring = tf.Print(arg_wiring, [arg_wiring],
            #    'op {} arg {} wiring: '.format(
            #      self._loom_op_names[op_idx], arg_idx))
            arg_input = tensor_arrays[arg_ts_idx].gather(arg_wiring)
        ...
        with tf.name_scope('scatter'):
          num_outputs = tf.shape(arg_inputs[0])[0]
          for output_idx, (output, output_ts) in enumerate(
              zip(op_outputs, op.output_type_shapes)):
            self._sanity_check_op_output(op_idx, output_idx, output, output_ts)

            # Place this outtput of this op to the appropriate places in the
            # corresponding TensorArray.
            ts_idx = self._type_shape_to_idx[output_ts]
            tensor_arrays[ts_idx] = tensor_arrays[ts_idx].scatter(
                tf.range(output_offsets[ts_idx],
                         output_offsets[ts_idx] + num_outputs),
                output,
                name=self._type_shapes[ts_idx].tensor_flow_name())

            output_offsets[ts_idx] += num_outputs

    return tensor_arrays + output_offsets
...
class Weaver(object):
  ...
  def build_feed_dict(self, outputs=None):
    ...
    if self._loom._direct_feed_dict:
      ...
      feed_dict[self._loom._num_results_per_type_shape] = (
          self._weaver.GetNumResultsPerTypeShape())
      return feed_dict
    else:
